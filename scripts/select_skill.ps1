...
$rules = @(
    ...
    [pscustomobject]@{ name = 'x64dbg-reverse'; pattern = 'x64dbg|x32dbg|\boep\b|\u52a8\u6001\u8c03\u8bd5|\u65ad\u70b9|\u5185\u5b58.{0,3}\u8865\u4e01|\u9644\u52a0.{0,6}(\u8fdb\u7a0b|process)|attach.*process|\u8131\u58f3'; confidence = 0.89; reason = 'Task text matches x64dbg dynamic debugging, breakpoint tracing, memory patching, process attach, or OEP/unpack workflow.' },
    [pscustomobject]@{ name = 'ce-reverse'; pattern = 'cheat\s*engine|cheatengine|(^|[^a-z0-9])ce([^a-z0-9]|$)|\u6307\u9488\u94fe|\u5185\u5b58\u626b\u63cf|\u626b\u63cf\u5185\u5b58|\u627e\u57fa\u5740|\u4f5c\u5f0a\u8868|speedhack|dbvm'; confidence = 0.89; reason = 'Task text matches Cheat Engine memory scanning, pointer-chain tracing, cheat table, or speedhack/DBVM workflow.' },
    ...
    [pscustomobject]@{ name = 'osint-recon'; pattern = 'osint|geoint|socmint|\u5f00\u6e90\u60c5\u62a5|\u60c5\u62a5(\u641c\u96c6|\u6536\u96c6)|\u4eba\u8089|\u793e\u5de5\u5e93|\u6570\u636e\u6cc4\u9732|\u6697\u7f51|(\u7528\u6237\u540d|\u90ae\u7bb1|\u624b\u673a\u53f7|\u57df\u540d).{0,4}\u53cd\u67e5|\u53cd\u67e5.{0,4}(\u7528\u6237\u540d|\u90ae\u7bb1|\u624b\u673a\u53f7|\u57df\u540d)'; confidence = 0.87; reason = 'Task text matches OSINT/recon tool lookup (username/domain/email/phone reverse lookup, data-breach search, dark web, social-engineering database).' },
