# ops — reverse-engineering-workflow 作战契约层
...
### 参考文件（ops/field-journal）落点：references/ 子目录

- source: `20260825-051545-上游-sidecar-参考文件-ops-field-journal-导入落点-reference`
- category: other
- applies_to: reference files organization
- purpose_zh: ops 作战契约层与 field-journal 实战日志并非模块，不应注册为 skill，而应整目录存放于 references/ops/ 与 references/field-journal/，并在 manifest.json references 中登记入口文件，保持与模块树分离。
- confidence: 4/5

**Lesson**

非模块类资产（契约文档、实战案例库）用 references/ 子目录整目录存放并登记 manifest，不注册为 skill；实战日志与 seed 案例保留原文件名便于日后检索复用。

**Evidence**

ops 10 文件 + field-journal 44 文件（17 篇实战日志 + 17 seed 案例）存放于 references/ 子目录，manifest-paths 校验通过，healthcheck 全部 PASS。

**Validation**

manifest-paths 校验通过；healthcheck 全部 PASS