...
$scriptRefs = @{
    ...
    'rasm2' = @('radare2/MODULE.md')
    'radiff2' = @('radare2/MODULE.md')
    'rahash2' = @('radare2/MODULE.md')
    'rax2' = @('radare2/MODULE.md')
    'python' = @('apk-reverse/scripts/frida-run.ps1')
    'pip' = @()
    'node' = @('reverse-engineering/MODULE.md')
    'npx' = @('reverse-engineering/MODULE.md')
    'jshookmcp' = @('reverse-engineering/MODULE.md')
    'agent-browser' = @('../../SKILL.md')
    'playwright' = @('../../SKILL.md')
    'analyzeHeadless' = @('reverse-engineering/MODULE.md')
    'proxycat' = @('../../security-research-modules/skills/recon-for-sec/MODULE.md')
    'nmap' = @('../../security-research-modules/skills/recon-for-sec/MODULE.md')
}

$reports = suimiGet-ReverseToolReport
...
$markdownLines = @(
    ...
    '',
    "- 扫描时间: $generatedAt",
    '- 路由入口: `SKILL.md` → `routing.md` → 对应内部模块',
...
foreach ($capName in $capabilityNames) {
    $state = suimiGet-ReverseCapabilityState -Name $capName
    if ($null -eq $state) { continue }
    $toolAvailable = $false
    try {
        $toolSpec = suimiResolve-ReverseToolSpec -Name $capName
