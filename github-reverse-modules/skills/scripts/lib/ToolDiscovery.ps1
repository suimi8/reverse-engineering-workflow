...
function suimiGet-ReverseToolCatalog {
    [CmdletBinding()]
    param()
    ...
    return @(
        ...
        [pscustomobject]@{
            Name = 'agent-browser'
            Skill = 'reverse-engineering-workflow'
...
function suimiGet-ReverseSkillRoot {
    [CmdletBinding()]
    param()
...
function suimiResolve-ReversePathTemplate {
    [CmdletBinding()]
    param(
        [AllowNull()]
        [string]$Value
    ...
    $replacements = @{
        ...
        '%APPDATA%' = $env:APPDATA
        '%TEMP%' = $env:TEMP
        '%SKILL_ROOT%' = suimiGet-ReverseSkillRoot
    }
...
function suimiGet-ReverseBootstrapManifestPath {
    [CmdletBinding()]
    param()

    return Join-Path (suimiGet-ReverseSkillRoot) 'scripts\bootstrap-manifest.json'
}

function suimiGet-ReverseBootstrapCatalog {
    [CmdletBinding()]
    param()
    ...
    $path = suimiGet-ReverseBootstrapManifestPath
    ...
    foreach ($capability in @($json.capabilities)) {
        ...
        foreach ($property in $capability.PSObject.Properties) {
            ...
            if ($value -is [string]) {
                $value = suimiResolve-ReversePathTemplate -Value $value
            }
            elseif ($value -is [System.Collections.IEnumerable] -and -not ($value -is [string])) {
                $items = @()
                foreach ($item in $value) {
                    if ($item -is [string]) {
                        $items += suimiResolve-ReversePathTemplate -Value $item
                    }
                    else {
                        $items += $item
                    }
                ...
                if ($propCount -gt 0) {
                    ...
                    foreach ($subProperty in $value.PSObject.Properties) {
                        ...
                        if ($subValue -is [string]) {
                            $subValue = suimiResolve-ReversePathTemplate -Value $subValue
                        }
...
function suimiGet-ReverseBootstrapDefinition {
    [CmdletBinding()]
    param(
    ...
    return suimiGet-ReverseBootstrapCatalog | Where-Object { $_.name -eq $Name } | Select-Object -First 1
}

function suimiGet-ClaudeMcpConfigPath {
    [CmdletBinding()]
    param()
...
function suimiGet-ClaudeMcpServerNames {
    [CmdletBinding()]
    param()

    $configPath = suimiGet-ClaudeMcpConfigPath
...
function suimiTest-ReverseTcpPort {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [int]$Port,
...
function suimiGet-ReverseCapabilityState {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$Name
    ...
    if ($definition.PSObject.Properties['mcpNames']) {
        $registeredNames = suimiGet-ClaudeMcpServerNames
    ...
    if ($definition.PSObject.Properties['servicePort'] -and $definition.servicePort) {
        $serviceOnline = suimiTest-ReverseTcpPort -Port ([int]$definition.servicePort)
    }
...
function suimiGet-ReverseToolDefinition {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$Name
    ...
    $definition = suimiGet-ReverseToolCatalog | Where-Object { $_.Name -eq $Name } | Select-Object -First 1
    if ($null -eq $definition) {
        throw "Unknown reverse tool: $Name"
    }

    $bootstrap = suimiGet-ReverseBootstrapDefinition -Name $Name
...
function suimiResolve-ReverseToolSpec {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$Name
    ...
    $definition = suimiGet-ReverseToolDefinition -Name $Name
    ...
    foreach ($candidate in $definition.Fallbacks) {
        switch ($candidate.Type) {
            ...
            'java-jar' {
                if (Test-Path -LiteralPath $candidate.Value) {
                    $java = suimiResolve-ReverseToolSpec -Name 'java'
                    return [pscustomobject]@{
                        Name = $definition.Name
                        Skill = $definition.Skill
                        Purpose = $definition.Purpose
...
function suimiGet-ReverseToolVersion {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [pscustomobject]$Spec
...
function suimiInvoke-ReverseTool {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$Name,
    ...
    $spec = suimiResolve-ReverseToolSpec -Name $Name
    if (-not $spec.Available) {
...
function suimiGet-ReverseToolReport {
    [CmdletBinding()]
    param(
        [string[]]$Names
    )

    $selectedNames = if ($null -ne $Names -and @($Names).Count -gt 0) { $Names } else { (suimiGet-ReverseToolCatalog).Name }

    foreach ($name in $selectedNames) {
        $spec = suimiResolve-ReverseToolSpec -Name $name
        $capabilityState = suimiGet-ReverseCapabilityState -Name $name
        [pscustomobject]@{
            Name = $spec.Name
            Skill = $spec.Skill
            ...
            Source = $spec.Source
            Version = suimiGet-ReverseToolVersion -Spec $spec
