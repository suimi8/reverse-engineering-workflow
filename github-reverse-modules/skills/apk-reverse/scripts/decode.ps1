...
function suimiGet-DefaultOutRoot {
...
function suimiGet-ManifestPackage {
...
if (-not $SkipJadx) {
    $jadxSpec = suimiResolve-ReverseToolSpec -Name 'jadx'
...
if (-not $SkipApktool) {
    $apktoolSpec = suimiResolve-ReverseToolSpec -Name 'apktool'
...
$taskName = if ($Name) { $Name } else { suimiGet-SafeName -PathValue $ApkPath }
$resolvedOutRoot = if ([string]::IsNullOrWhiteSpace($OutRoot)) { suimiGet-DefaultOutRoot -ApkFilePath $ApkPath } else { $OutRoot }
...
$packageName = if (-not $SkipApktool) { suimiGet-ManifestPackage -ManifestPath $manifestPath } else { '' }
