...
foreach ($perm in $permissions) {
    $name = suimiGet-AndroidAttr -Node $perm -Name 'name'
    if ($name) {
        "permission=$name"
    }
}
...
foreach ($query in $componentQueries) {
    ...
    foreach ($node in $nodes) {
        $name = suimiGet-AndroidAttr -Node $node -Name 'name'
        $exported = suimiGet-AndroidAttr -Node $node -Name 'exported'
        $enabled = suimiGet-AndroidAttr -Node $node -Name 'enabled'
...
foreach ($activity in $activities) {
    ...
    foreach ($filter in $filters) {
        $hasMain = @($filter.SelectNodes('action')).Where({ (suimiGet-AndroidAttr -Node $_ -Name 'name') -eq 'android.intent.action.MAIN' }).Count -gt 0
        $hasLauncher = @($filter.SelectNodes('category')).Where({ (suimiGet-AndroidAttr -Node $_ -Name 'name') -eq 'android.intent.category.LAUNCHER' }).Count -gt 0
        if ($hasMain -and $hasLauncher) {
            $mainActivities += (suimiGet-AndroidAttr -Node $activity -Name 'name')
        }
    }
}
