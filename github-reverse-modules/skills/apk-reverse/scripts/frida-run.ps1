...
function suimiGet-PythonScriptCandidates {
...
function suimiGet-ToolPath {
    ...
    foreach ($candidate in suimiGet-PythonScriptCandidates -ExecutableName ($Name + '.exe')) {
        if (Test-Path -LiteralPath $candidate) {
            return $candidate
        }
    }
...
$fridaLsDevices = suimiGet-ToolPath -Name 'frida-ls-devices'
$fridaPs = suimiGet-ToolPath -Name 'frida-ps'
$frida = suimiGet-ToolPath -Name 'frida'
