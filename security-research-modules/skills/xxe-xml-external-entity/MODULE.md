...
- [upload insecure files](../upload-insecure-files/MODULE.md) when XXE is reachable through SVG, OOXML, import, or preview pipelines
...
```
WINDOWS_HOSTS_FILE
WINDOWS_WEBROOT\web.config    ← ASP.NET connection strings
WINDOWS_PHP_APP\wp-config.php    ← WordPress DB credentials
WINDOWS_USER_HOME\.ssh\id_rsa
```
...
```
WINDOWS_DTD_DIR\cim20.dtd            # WMI
WINDOWS_DTD_DIR\wmi20.dtd            # WMI
WINDOWS_WEBSPHERE_DTD\*.dtd          # WebSphere
WINDOWS_LOTUS_DTD\*.dtd              # Lotus Notes
```
