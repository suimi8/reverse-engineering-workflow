...
- **输入进入解释器/查询语言/模板**（与 HTTP 分帧无关）→ [Injection Testing Router](../injection-checking/MODULE.md)（再下钻 XSS、SQLi、SSTI 等）。
- **响应头拆分、Location CRLF** → [CRLF Injection](../crlf-injection/MODULE.md)。
- **缓存与路径键混淆** → [Web Cache Deception](../web-cache-deception/MODULE.md)。
