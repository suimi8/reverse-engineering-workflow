...
- [upload insecure files](../upload-insecure-files/MODULE.md) when the primary attack surface is an upload workflow rather than an include or read primitive
...
WINDOWS_HOSTS_FILE
WINDOWS_BOOT_CONFIG
WINDOWS_ACCOUNT_DATABASE          ← NTLM hashes (often locked)
WINDOWS_WEBROOT\web.config        ← ASP.NET DB connection strings
WINDOWS_WEBROOT\global.asa
WINDOWS_PHP_APP\wp-config.php
WINDOWS_USER_HOME\.ssh\id_rsa
WINDOWS_MYSQL_CONFIG              ← MySQL config
```
