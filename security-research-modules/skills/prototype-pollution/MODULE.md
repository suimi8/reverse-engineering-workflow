...
- 输入路由与多类注入并列入口 → [Injection Testing Router](../injection-checking/MODULE.md)。
- 模板执行链（非 PP）→ [SSTI](../ssti-server-side-template-injection/MODULE.md)。
- 不安全反序列化（非 JS 原型）→ [Deserialization](../deserialization-insecure/MODULE.md)。
